...
{
    include 'conn.php';
    $q4 = "SELECT id, calle_norm, altura_norm FROM sig.mapa.diruni where estado = 2";
    ...
    while($row4=pg_fetch_array($res4,NULL,PGSQL_ASSOC) )
        {
            ...
            $json4 = file_get_contents($peticion4, False);
            $json4_output = json_decode($json4);
            if(empty($json4_output))
            {
                $q6 = "UPDATE sig.mapa.diruni SET estado = 3 WHERE id = " . $elaidi;
            ...
            else
            {
                ...
                // if (empty($json4_output->codigo_planeamiento_urbano)){$codigo_planeamiento_urbano = null;}else{$ccodigo_planeamiento_urbano = $json4_output->codigo_planeamiento_urbano;};
                // if (empty($json4_output->seccion_catastral)){$seccion_catastral = null;}else{$seccion_catastral = $json4_output->seccion_catastral;};
                // if (empty($json4_output->distrito_economico)){$distrito_economico = null;}else{$distrito_economico = $json4_output->distrito_economico;};
                // if (empty($json4_output->codigo_postal)){$codigo_postal = null;}else{$codigo_postal = $json4_output->codigo_postal;};
                ...
                // . "', codigo_planeamiento_urbano = '" . $codigo_planeamiento_urbano
                // . "', seccion_catastral = '" . $seccion_catastral
                // . "', distrito_economico = '" . $distrito_economico                
                // . "', codigo_postal = '" . $codigo_postal
                . "', cpa = '" . $codigo_postal_argentino
                . "', estado = 4 "
                . "WHERE id = " . $elaidi;
...
{
    include 'conn.php';
    $q10 = "SELECT id, codigo_calle, altura_norm FROM sig.mapa.diruni WHERE codigo_calle is not null";
